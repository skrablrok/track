...
import { Building2, Lock, AlertCircle, ChevronDown, Eye, EyeOff, CheckCircle } from 'lucide-react'
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok || data.error) {
        setError(data.error === 'invalid_token' ? t(lang, 'resetTokenInvalid') : t(lang, 'networkError'))
      } else {
        setSuccess(true)
      }
    } catch {
        ...
        <div className="bg-white rounded-3xl shadow-2xl p-8">
          ...
          {success ? (
            <div className="text-center space-y-4">
              ...
              <p className="text-sm text-gray-600">{t(lang, 'resetPasswordSuccess')}</p>
              <button
                onClick={() => router.push('/login')}
                className="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 px-4 rounded-xl transition-all shadow-sm mt-2"
              >
                {t(lang, 'signIn')}
              </button>
            </div>
            ...
            <>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  ...
                  <p className="text-xs text-gray-400 mb-1.5">{t(lang, 'passwordMin8')}</p>
                ...
                <button type="submit" disabled={loading}
                  ...
                  {loading ? t(lang, 'resetting') : t(lang, 'resetPassword')}
                </button>
              </form>
